...
`));
const px_to_vw = (px2) => px2 / window.innerWidth * 100;
const px_to_vh = (px2) => px2 / window.innerHeight * 100;
const windowwidth = sig(window.innerWidth);
const scale = mem(() => windowwidth() > 1150 ? 1 : 0.75);
const mobile = mem(() => windowwidth() < 1e3 ? true : false);
...
let sections = [
  ...
  {title: "Panel (3)"},
  {
    title: "Yiu Hei {Myfriends studio}",
    time: "7:40pm",
...
const shuffle = () => {
  ...
  options.forEach((el, index) => {
    ...
    let last_end = first ? 0 : positions[last_index].x + positions[last_index].w;
  ...
  let done = () => {
    ...
    if (s_index == 0)
      return true;
    if (options[s_index - 1] != "about")
      return true;
    else
      return false;
  };
  do {
    options.sort((_) => Math.random() > 0.5 ? 1 : -1);
  } while (!done());
};
...
const Stage = (() => {
  ...
  const css2 = [".canvas", {cursor: "col-resize", position: "fixed", "mix-blend-mode": "difference"}, fullscreen];
