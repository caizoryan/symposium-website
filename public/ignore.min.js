...
function init_p5(el) {
  ...
  p2.setup = () => {
    p2.createCanvas(window.innerWidth, window.innerHeight, {alpha: true});
...
let purple_grid = compose({
  css: () => CSS.css({"box-shadow": [[0, 0, px(30), px(10), colors.black + "22"]]})
...
function Dual(letter, rectangle, materials2, font = "cirrus", size = em(3.7)) {
...
const shape = (src, fn, dim_mul = 12) => {
  let {x, y} = offscreen();
  let rectangle = new Rectangle(x, y, Math.random() * dim_mul + 2, Math.random() * dim_mul + 2, {
    unit: "v",
...
function layer_one_shapes() {
  let shapes = Array(12).fill(0).map((e, i) => shape("./shapes/shape_" + (i + 1) % 7 + ".png", randomizer));
  shapes.forEach((e) => {
...
let alts = a.map((char, i) => {
  ...
  let rect2 = new Rectangle(i * lexlen, offset(1), lexlen, 10, {unit: "v", strategy: "absolute"});
  return Dual(char, rect2, randos);
});
let alt_cont = container(new Rectangle(alternate_position.x, alternate_position.y, 60, 20, {material: {css: () => `transform: scale(1.7) rotate(2deg);`}}), ...alts);
space.add(alt_cont);
let p = ["Prac", "tices"];
let prac = p.map((char, i) => {
  ...
  let rect2 = new Rectangle(i * lexlen, 0, lexlen, 10, {unit: "v"});
  return Dual(char, rect2, randos);
});
let prac_cont = container(new Rectangle(practices_position.x, practices_position.y, 60, 20, {material: {css: () => `transform: scale(1.6) rotate(-1.5deg);`}}), ...prac);
space.add(prac_cont);
let sym = ["SYMPO", "SIUM"];
let symy = sym.map((char, i) => {
  ...
  let rect2 = new Rectangle(i * lexlen, 0, lexlen, 7, {unit: "v"});
  return Dual(char, rect2, randos, "roberte");
});
let sym_cont = container(new Rectangle(symposium_position.x, symposium_position.y, 60, 12, {material: {css: () => `transform: scale(1.5) rotate(-.5deg);`}}), ...symy);
space.add(sym_cont);
function layer_two_shapes() {
  let shapes = Array(3).fill(0).map((e, i) => shape("./shapes/shape_" + (i + 4) % 7 + ".png", randomizer, 5));
  shapes.forEach((e) => {
