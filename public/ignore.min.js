...
function Rectangle(x, y, w, h, opts) {
  ...
  const _unit = (prop) => {
    let def = (axis) => _opts().unit == "v" ? axis == "x" ? "vw" : "vh" : _opts().unit;
    if (prop == "x")
      return _opts().xUnit ? _opts().xUnit : def("x");
    if (prop == "y")
      return _opts().yUnit ? _opts().yUnit : def("y");
    if (prop == "w")
      return _opts().wUnit ? _opts().wUnit : def("x");
    if (prop == "h")
      return _opts().hUnit ? _opts().hUnit : def("y");
  };
  const api = {
    x: _x,
    y: _y,
    ...
    add_child,
    opts: _opts,
    css: () => mem(() => css(rect(_x() + _unit("x"), _y() + _unit("y"), _w() + _unit("w"), _h() + _unit("h"), _opts())))
  };
  let navigator = Navigator(api);
  api.navigator = navigator;
  eff_on(navigator.destination, () => children.forEach((child) => child.follow(Object.assign(derived(), navigator.destination()))));
  return api;
}
...
let masked = Child(dom, (dims) => {
  setTimeout(() => {
    let actual = () => dom.rectangle.navigator.navigate_to(dims.x + offset(3), dims.y + offset(4) - 2, 8, 250);
    let jumpy = () => {
