...
function init_p5(el) {
  ...
  p.draw = () => {
    ...
    let rw = p.width / 4;
    let ry = p.height / 4;
    r1 = rw * 0.8 + p.cos(90 - an) * (rw * 0.2);
    r2 = ry * 0.8 + p.sin(90 - an) * (ry * 0.2);
  };
}
...
const Information = (() => {
  ...
  let rectangle = new Rectangle(40, 1, 100 - 40 - 1, 60, {unit: "v", material});
...
setInterval(() => {
  ...
  comps.forEach((el) => {
    ...
    if (out) {
      ...
      pos = random_pos(el.rectangle.w(), el.rectangle.h());
    }
